function convert() {
  ...
  while (input != preinput) {
    ...
    input = input.replace(/(korarr_[0123456789_]+) 의 ([첫한두세네다섯여섯일곱여덟아홉열스물무서른마흔쉰예순일흔여든아흔영일이삼사오육칠팔구십백천만억조]+) (번째|번|째) (원소|수)/g, arrelementreplacer)
    input = input.replace(/(korarr_[0123456789_]+) 의 ([0-9\+\-\*/_korvar\[\]\(\)]+) (번째|번|째) (원소|수)/g, '$1[$2]')
  }
